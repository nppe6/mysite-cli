...
const resolveTemplate = async (prompt, framework) => {
  ...
  return selectedTemplate.variants.find((item) => item.name === variant)
}

const myAction = async (project, args) => {
  const prompt = inquirer.createPromptModule()
  const selectedTemplate = await resolveTemplate(prompt, args.framework)

  downloadFun(selectedTemplate.url, project, selectedTemplate)
}
